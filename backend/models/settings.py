...
class Settings(db.Model):
    ...
    # MiniMax API 配置
    minimax_api_key = db.Column(db.String(500), nullable=True)   # MiniMax API Key（当前主要使用）
    minimax_api_base = db.Column(db.String(500), nullable=True)  # MiniMax API Base URL
    text_model = db.Column(db.String(100), nullable=True)  # 文本大模型名称（如 MiniMax-M2.7）
    ...
    kimi_api_key = db.Column(db.String(500), nullable=True)  # Kimi API Key
    ...
    def to_dict(self):
        ...
        return {
            ...
            'minimax_api_key_length': len(self._val('minimax_api_key', d) or '') if self._val('minimax_api_key', d) else 0,
            'minimax_api_base': self._val('minimax_api_base', d),
            ...
            'kimi_api_key_length': len(self._val('kimi_api_key', d)) if self._val('kimi_api_key', d) else 0,
    ...
    def _get_config_defaults():
        ...
        return {
            ...
            'api_base_url': Config.OPENAI_API_BASE if Config.AI_PROVIDER_FORMAT == 'openai' else (Config.GOOGLE_API_BASE or None),
            'api_key': Config.OPENAI_API_KEY if Config.AI_PROVIDER_FORMAT == 'openai' else (Config.GOOGLE_API_KEY or None),
