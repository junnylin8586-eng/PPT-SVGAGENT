...
class Project(db.Model):
    ...
    # PPT Agent 新增字段
    name = db.Column(db.String(200), nullable=True)  # 项目名称
    template_path = db.Column(db.String(500), nullable=True)  # e.g. 'government_blue'
    generation_mode = db.Column(db.String(20), nullable=False, default='guide')  # auto | guide | edit
    is_deleted = db.Column(db.Boolean, nullable=False, default=False)  # 软删除
    primary_color = db.Column(db.String(10), nullable=True, default='#003371')  # 主色调
    font_family = db.Column(db.String(100), nullable=True, default='system-ui')  # 字体
    layout_style = db.Column(db.String(20), nullable=True, default='balanced')  # balanced|compact|spacious
    ...
    def to_dict(self, include_pages=False):
        ...
        data = {
            ...
            'template_image_url': f'/files/{self.id}/template/{self.template_image_path.replace(chr(92), "/").split("/")[-1]}' if self.template_image_path else None,
        ...
        if include_pages:
            ...
            data['pages'] = [page.to_dict() for page in self.pages if not page.is_deleted]
