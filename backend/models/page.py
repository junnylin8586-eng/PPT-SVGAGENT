...
class Page(db.Model):
    ...
    elements = db.Column(db.Text, nullable=True)  # JSON array of canvas elements
    ...
    code_block_id = db.Column(db.String(100), nullable=True)  # 代码块 ID（可选）
    status = db.Column(db.String(50), nullable=False, default='DRAFT')
    # PPT Agent 新增字段
    svg_path = db.Column(db.String(500), nullable=True)  # SVG 文件路径
    ...
    def set_elements(self, data):
        """Set elements as JSON string"""
        if data:
            self.elements = json.dumps(data, ensure_ascii=False)
        else:
            self.elements = None
