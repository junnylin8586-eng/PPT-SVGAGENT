...
from .prompts import get_layered_image_generation_prompt
from services.vlm_analyzer import VLMAnalyzer


def _enrich_all_descriptions(app, project_id: str, project_context, language: str = 'zh'):
    """
    为所有页面补充具体内容细节（第二遍 pass）。

    流程：
    1. 读取所有已生成的页面描述
    2. 对每页调用 LLM 补充具体细节（历史背景、数据、案例等）
    3. 结合上下页面内容进行连贯性补充
    4. 更新每页的 description_content
    5. 重新生成 page_elements（基于更丰富的描述）
    """
    from utils.page_utils import auto_generate_page_elements
    ...
    with app.app_context():
        ...
        logger.info(f"[Enrichment] Starting for project {project_id}, {len(pages)} pages")

        # 获取项目背景信息
        project_context_text = None
        if project_context:
            parts = []
            if project_context.idea_prompt:
                parts.append(project_context.idea_prompt)
            if project_context.outline_requirements:
                parts.append(project_context.outline_requirements)
            if parts:
                project_context_text = "\n".join(parts)

        # 获取 AI service（需要新的 instance，避免跨线程问题）
        ...
        for i, page_info in enumerate(page_list):
            ...
            if not page_content or len(page_content.strip()) < 10:
                logger.info(f"[Enrichment] Page {i+1} has no content, skipping")
                continue

            # 判断是否需要 enrichment（内容过短/过简）
            # 如果字数少于80字，认为是骨架状态，需要补充
            if len(page_content) > 200:
                logger.info(f"[Enrichment] Page {i+1} content sufficient ({len(page_content)} chars), skipping")
                continue
            ...
            next_info = None
            if i > 0:
                prev_info = {'title': page_list[i-1]['title'], 'content': page_list[i-1]['content']}
            if i < len(page_list) - 1:
                next_info = {'title': page_list[i+1]['title'], 'content': page_list[i+1]['content']}

            try:
                # 调用 enrichment
                enriched_text = ai_service.enrich_page_description(
                ...
                if enriched_text and len(enriched_text) > len(page_content):
                    # 更新 description_content
                    desc_content = page.get_description_content() or {}
                    desc_content['text'] = enriched_text
                    desc_content['enriched_at'] = datetime.utcnow().isoformat()
                    page.set_description_content(desc_content)

                    # 重新生成 page_elements（基于更丰富的描述）
                    outline_content = page.get_outline_content()
                    elements = auto_generate_page_elements(outline_content, desc_content)
                    page.set_elements(elements)

                    logger.info(f"[Enrichment] Page {i+1} enriched: {len(page_content)} → {len(enriched_text)} chars")
                    enriched_count += 1
                else:
                    logger.info(f"[Enrichment] Page {i+1} no improvement, skipping update")

            except Exception as e:
                logger.warning(f"[Enrichment] Page {i+1} failed: {e}")

        logger.info(f"[Enrichment] Completed: {enriched_count}/{len(pages)} pages enriched")
...
# 检查生成结果是否包含文字（使用 VLM）
def check_image_for_text(image: Image.Image, api_key: str = None) -> bool:
    """
    使用 MiniMax VLM 检测图片是否包含文字。
    
    Args:
        image: PIL Image 对象
        api_key: 可选，明确传入 API key
    ...
    try:
        ...
        # 保存到临时文件（VLMAnalyzer 需要文件路径）
        with tempfile.NamedTemporaryFile(suffix='.png', delete=False) as f:
            image.save(f.name, format='PNG')
            temp_path = f.name
        try:
            result = vlm.analyze_image(temp_path)
            elements = result.get('elements', [])
            for el in elements:
                if el.get('type') == 'text' and el.get('text_content'):
        ...
        logger.warning(f"[VLM text check] Failed: {e}")
        return False
...
def generate_descriptions_task(task_id: str, project_id: str, ai_service,
    ...
    with app.app_context():
        try:
                ...
                for future in as_completed(futures):
                    ...
                    if page:
                        if error:
                            ...
                            # Fix: 同时生成 page_elements，避免 description_content 有文字但 elements 为空
                            outline_content = page.get_outline_content()
                            elements = auto_generate_page_elements(outline_content, desc_content)
                            page.set_elements(elements)
                            completed += 1
            ...
            # ── 内容补充（Enrichment）Pass ──────────────────────────────────────
            # 第二遍：为每页补充具体细节（需要所有页面的描述已生成）
            try:
                _enrich_all_descriptions(
                    app=app,
                ...
                logger.warning(f"Enrichment failed for project {project_id}: {enrich_error}")
            ...
            def generate_single_image(page_id, page_data, page_index):
                ...
                with app.app_context():
                    try:
                        ...
                        # 【改造要点】获取 page_elements 用于图片/文字分离生成
                        # PPT-MASTER 风格:图片只生成视觉背景,文字通过 page_elements 在导出时渲染
                        page_elements = page_obj.get_elements()

                        # P1: 如果 elements 为空,自动从 outline_content 构建元素
                        # 这样图片生成得到纯背景(prompt 知道文字在哪里),
                        # 导出时也能用精确布局渲染文字
                        outline_content = page_obj.get_outline_content()
                        if not page_elements and outline_content:
                            page_elements = auto_generate_page_elements(outline_content, desc_content)
                            logger.info(f"[P1] Auto-generated {len(page_elements)} elements for page {page_id}")
                        ...
                        # 【改造要点】传递 page_elements,实现图片/文字分离生成
                        # PPT-MASTER 风格:图片生成纯背景设计,文字通过 page_elements 在导出时渲染
                        ...
                        # 检查生成结果是否包含文字（如果有 page_elements，说明走了图片/文字分离流程）
                        # 若检测到文字，说明 AI 生成失败了，需要重新生成（不带模板图）
                        if page_elements and len(page_elements) > 0:
                            has_text = check_image_for_text(image)
                            if has_text:
                                logger.warning(f"[Page {page_id}] VLM detected text in generated image, retrying without template...")
                                # 重试：不传模板图，has_template=False
                                image = ai_service.generate_image(
                                    prompt, None, aspect_ratio, resolution,
                                    additional_ref_images=page_additional_ref_images if page_additional_ref_images else None
                                )
                                if not image:
                                    raise ValueError("Failed to generate image (retry)")
                                logger.info(f"[Page {page_id}] Retry successful")
                        ...
                        # P1: 保存自动生成的 page_elements 到数据库
                        # 供后续 exportModularPPTX 使用精确布局
                        if page_elements:
                            page_obj.set_elements(page_elements)
                            db.session.commit()
                ...
                for future in as_completed(futures):
                    ...
                    if task:
                        ...
                        if resolution_mismatched > 0 and 'warning_message' not in progress:
                            progress['warning_message'] = f"图片返回分辨率与设置不符({resolution_mismatched}/{len(pages)} 页),部分 AI 图片提供商会因最小像素限制微调尺寸,实际效果不受影响"
...
def generate_single_page_image_task(task_id: str, project_id: str, page_id: str,
                                    ...
                                    use_template: bool = False, aspect_ratio: str = "16:9",
                                    resolution: str = "2K", app=None,
                                    extra_requirements: str = None,
                                    language: str = None):
    """
    ...
    Note: app instance MUST be passed from the request context.
    use_template 默认为 False(不参考模板图),避免模板风格污染生成结果。
    """
    if app is None:
        raise ValueError("Flask app instance must be provided")
    ...
    with app.app_context():
        try:
            ...
            # 读取 canvas 元素数据,传递给生成器以渲染正确的文字内容
            # FIX: 强制重新生成 elements，不使用数据库中可能存在的错误格式 elements
            page_elements = None
            
            # P1: 自动从 outline_content 构建元素
            outline_content = page.get_outline_content()
            if outline_content:
                page_elements = auto_generate_page_elements(outline_content, desc_content)
                logger.info(f"[P1] Auto-generated {len(page_elements)} elements for page {page_id}")

            logger.info(f"[generate_single_page_image_task] page_id={page_id}, elements_count={len(page_elements)}")
            ...
            if page_elements and len(page_elements) > 0:
                has_text = check_image_for_text(image)
                if has_text:
                    logger.warning(f"[Page {page_id}] VLM detected text in generated image, retrying with stricter prompt...")
                    # 重试时使用更严格的 prompt，加强无文字约束
                    strict_prompt = prompt + "\n\n[STRICT ENFORCEMENT] This image MUST be pure abstract decoration ONLY. ZERO text allowed. If you include text, the image will be rejected. Generate only: soft gradients, light effects, flowing lines, geometric shapes. NO words, NO letters, NO numbers, NO symbols."
                    image = ai_service.generate_image(
                        strict_prompt, None, aspect_ratio, resolution,
                        additional_ref_images=additional_ref_images if additional_ref_images else None
                    )
                    if not image:
                        raise ValueError("Failed to generate image (retry)")
                    # 再次检查
                    has_text_retry = check_image_for_text(image)
                    if has_text_retry:
                        logger.warning(f"[Page {page_id}] Retry still has text, trying one more time...")
                        image = ai_service.generate_image(
                            strict_prompt, None, aspect_ratio, resolution,
                            additional_ref_images=additional_ref_images if additional_ref_images else None
                        )
                        if not image:
                            raise ValueError("Failed to generate image (second retry)")
                    logger.info(f"[Page {page_id}] Retry successful")
            ...
            # P1: 保存自动生成的 page_elements 到数据库
            if page_elements:
                page.set_elements(page_elements)
                db.session.commit()
...
def generate_layered_page_images_task(
    task_id: str,
    project_id: str,
    page_id: str,
    ai_service,
    ...
    为页面生成多个无文字的图片层（background/decoration/graphic），
    用于在 PPTX 导出时叠加出更精美的效果。
    ...
    with app.app_context():
        try:
            ...
            # 获取页面标题
            oc = page.get_outline_content() or {}
            page_title = oc.get('title', '')

            # 获取模板图片（作为参考）
            ref_image_path = file_service.get_template_path(project_id)

            # 分辨率转换为像素
            px_map = {'2K': (2560, 1440), '1K': (1920, 1080), '4K': (3840, 2160)}
            px_size = px_map.get(resolution, (2560, 1440))

            # 生成分层图片
            layer_types = ['background', 'decoration', 'graphic']
            saved_layers = {}

            for layer_type in layer_types:
                try:
                    logger.info(f"🎨 Generating {layer_type} layer for page {page_id}...")

                    layer_image = ai_service.generate_layered_image(
                        page_title=page_title,
                    ...
                    if layer_image:
                        # 保存图层
                        layer_rel_path = file_service.save_layer_image(
                            layer_image, project_id, page_id, layer_type
                        )
                        saved_layers[layer_type] = layer_rel_path
                        logger.info(f"✅ {layer_type} layer saved: {layer_rel_path}")

                except Exception as layer_error:
                    logger.warning(f"⚠️ Failed to generate {layer_type} layer: {layer_error}")

            # 更新 description_content.extra_fields.image_layers
            if saved_layers:
                desc_content.setdefault('extra_fields', {})
                desc_content['extra_fields']['image_layers'] = saved_layers
                desc_content['layers_generated_at'] = datetime.utcnow().isoformat()
                page.set_description_content(desc_content)
                db.session.commit()
                logger.info(f"[Layered] Updated page {page_id} with layers: {list(saved_layers.keys())}")

            # 标记任务完成
            task.status = 'COMPLETED'
            task.completed_at = datetime.utcnow()
            task.set_progress({"total": 3, "completed": len(saved_layers), "failed": 3 - len(saved_layers)})
            db.session.commit()
            logger.info(f"✅ Layered task {task_id} COMPLETED - {len(saved_layers)} layers generated")

        except Exception as e:
            import traceback
            error_detail = traceback.format_exc()
            logger.error(f"Layered task {task_id} FAILED: {error_detail}")
