...
class FileService:
    ...
    def save_layer_image(self, image: Image.Image, project_id: str,
                          page_id: str, layer_type: str,
                          version_number: int = None) -> str:
        """
        保存分层图片元素（background/decoration/graphic）

        Args:
            image: PIL Image object
            project_id: Project ID
            page_id: Page ID
            layer_type: 图层类型（background/decoration/graphic）
            version_number: Optional version number

        Returns:
            Relative file path from upload folder (e.g., "project_id/pages/page_id_layer_background.png")
        """
        pages_dir = self._get_pages_dir(project_id)
        ext = 'png'

        if version_number is not None:
            filename = f"{page_id}_layer_{layer_type}_v{version_number}.{ext}"
        else:
            import time
            timestamp = int(time.time() * 1000)
            filename = f"{page_id}_layer_{layer_type}_{timestamp}.{ext}"
